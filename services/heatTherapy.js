...
cron.schedule("*/5 * * * * *", async () => {
    try {
        ...
        if (compressorStatus === 0) {
            ...
            const lastRecord = await HeatTherapy
                .findOne({})
                .sort({ id: -1 });
            ...
            const record = await HeatTherapy.findOne({ patientId: activePatientId });
            ...
            await record.save();
            console.log("HeatTherapy saved for", activePatientId);
        }
